        ...
        server.route({
            ...
            handler: async (request, h) => {
                ...
                if (countError) {
                    ...
                    return h
                        .response({
                            message: "Failed to count outcome",
            ...
            handler: async (request, h) => {
                if (!request.auth.isAuthenticated)
                    return h
                        ...
                        .code(401);
                const userId = request.auth.credentials.id;
                const { kategori_id, jumlah, keterangan, tanggal } =
                    request.payload;
                ...
                if (kategori_id === undefined || kategori_id === null) {
                    return h
                        .response({ message: "Kategori ID is required" })
                        .code(400);
                }
            ...
            handler: async (request, h) => {
                ...
                if (!data || data.length === 0) {
                    return h
                        .response({
                            message:
                                "Outcome not found or not authorized to update",
                        })
                        .code(404);
                }
            ...
            handler: async (request, h) => {
                ...
                if (!data || data.length === 0) {
                    return h
                        .response({
                            message:
                                "Outcome not found or not authorized to delete",
                        })
                        .code(404);
                }
            ...
            handler: async (request, h) => {
                ...
                if (catError) {
                    return h
                        .response({
                            message:
                                "Could not fetch outcome categories for mapping.",
                ...
                for (const record of dataToImport) {
                    if (
                        ...
                        errors.push({
                            record,
                            error: "Missing Tanggal, Jumlah, Kategori, or Jumlah is not a number.",
                        });
                        continue;
                    }

                    const kategori_id =
                        categoryMap[record.Kategori.toLowerCase()];
                    if (!kategori_id) {
                        failedImports++;
                        errors.push({
                            record,
                            error: `Category "${record.Kategori}" not found.`,
                        });
                        continue;
                    }
