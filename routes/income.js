  ...
  register: async function (server, options) {
    ...
    server.route({
      ...
      handler: async (request, h) => {
        ...
        if (countError) { 
            ...
            return h.response({ message: 'Failed to count income', error: countError.message }).code(500);
        }
        ...
        if (dataError) { 
            ...
            return h.response({ message: 'Failed to fetch income', error: dataError.message }).code(500);
        }
    ...
    server.route({
      ...
      handler: async (request, h) => {
        ...
        if (error) { 
            ...
            return h.response({ message: 'Failed to fetch income categories', error: error.message }).code(500);
        }
    ...
    server.route({
      ...
      handler: async (request, h) => {
        ...
        const userId = request.auth.credentials.id;
        const { jumlah, keterangan, tanggal, kategori_id } = request.payload;
        ...
        const { data, error } = await supabase
          .from('pemasukan')
          .insert([{ user_id: userId, jumlah, keterangan, tanggal: tanggal || new Date().toISOString(), kategori_id }])
          .select('*, kategori_pemasukan(id, nama)');
    ...
    server.route({
        ...
        handler: async (request, h) => {
            ...
            if (!data || data.length === 0) {
                return h.response({ message: 'Income not found or not authorized to delete' }).code(404);
            }
    ...
    server.route({
        ...
        handler: async (request, h) => {
            ...
            const { data: categoriesDB, error: catError } = await supabase
                ...
                .select('id, nama');
            if (catError) {
                return h.response({ message: 'Could not fetch income categories for mapping.', error: catError.message }).code(500);
            }
            ...
            for (const record of dataToImport) {
                if (!record.Tanggal || !record.Jumlah || isNaN(parseFloat(record.Jumlah))) {
                    failedImports++;
                    errors.push({ record, error: 'Missing Tanggal or Jumlah, or Jumlah is not a number.' });
                    continue;
                }
                
                let kategori_id = null;
                if (record.Kategori && typeof record.Kategori === 'string') {
                    kategori_id = categoryMap[record.Kategori.toLowerCase()] || null;
                }
